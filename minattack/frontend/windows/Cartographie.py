...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QScrollArea, QApplication
...
class GraphWindow(QWidget):
    ...
    def __init__(self, graph_data, parent=None):
        ...
        try:
            ...
            logger.exception("GraphWindow [ERROR]: Erreur durant l'initialisation")
            raise
    ...
    def _setup_ui(self):
        ...
        title.setStyleSheet("font-size: 16px; font-weight: bold; color: #4CAF50; padding: 10px;")
        ...
        controls_label.setStyleSheet("font-weight: bold; margin-right: 10px; color: #fff;")
        ...
        self.info_label = QLabel("Cliquez sur un nœud pour afficher ses détails", self)
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'info_label'):
            ...
            margin = 20
            self.info_label.move(margin, self.height() - self.info_label.height() - margin)
    
    def closeEvent(self, event):
        """Événement de fermeture personnalisé"""
        logger.warning("GraphWindow [CLOSE]: Fermeture de la fenêtre graphique")
        event.accept()    
    ...
    def _parse_graph_data(self):
        ...
        logger.warning(f"GraphWindow [PARSE]: {len(self.nodes)} nœuds, {len(self.edges)} arêtes chargés")
    ...
    def drawNodeLabel(self, painter, node, x, y, size):
        ...
        painter.setFont(QFont("Arial", font_size, QFont.Bold if level == 0 else QFont.Normal))
        ...
        bg_rect.moveTopLeft(QPoint(text_x - 3, text_y - text_rect.height() + 2))
    ...
    def paintCanvas(self, event):
        ...
        try:
            ...
            if not self.nodes:
                ...
                painter.drawText(self.canvas.rect().center(), "Aucune donnée à afficher")
                return
            ...
            painter.translate(center_x + self.pan_offset_x, center_y + self.pan_offset_y)
    ...
    def extractPathSegment(self, full_url, level):
        ...
        try:
            ...
            logger.error(f"GraphWindow [ERROR]: Erreur lors de l'extraction du segment de chemin: {e}")
            return full_url
    ...
    def canvasMousePress(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if clicked_node:
                ...
                logger.warning(f"GraphWindow [CLICK]: Nœud sélectionné: {clicked_node['id']}")
                self.dragging = False
                self.drag_start_pos = None
            else:
                self.selected_node = None
    ...
    def canvasMouseWheel(self, event):
        ...
        before_x = (mouse_pos.x() - center_x - self.pan_offset_x) / self.zoom_factor
        before_y = (mouse_pos.y() - center_y - self.pan_offset_y) / self.zoom_factor
    ...
    def getNodeAtPosition(self, canvas_pos):
        ...
        for node in self.nodes:
            ...
            node_x = center_x + (node_pos['x'] * self.zoom_factor) + self.pan_offset_x
            node_y = center_y + (node_pos['y'] * self.zoom_factor) + self.pan_offset_y
            
            size = self.getNodeSize(node) * self.zoom_factor
            
            # Vérifier si le clic est dans le cercle du nœud
            distance = math.sqrt((canvas_pos.x() - node_x)**2 + (canvas_pos.y() - node_y)**2)
            if distance <= size:
                return node
    ...
    def loadNodePositions(self):
        ...
        if has_positions:
            ...
            logger.warning("GraphWindow [POSITIONS]: Génération des positions automatique")
    ...
    def centerGraph(self):
        ...
        if x_coords and y_coords:
            ...
            if graph_width > 0 and graph_height > 0:
                ...
                zoom_x = (canvas_width * 0.4) / graph_width if graph_width > 0 else 1.0
                zoom_y = (canvas_height * 0.4) / graph_height if graph_height > 0 else 1.0
                
                self.zoom_factor = min(zoom_x, zoom_y, 2.0)  # Limiter le zoom max à 2.0
                self.zoom_factor = max(0.3, self.zoom_factor)  # Limiter le zoom min à 0.3
