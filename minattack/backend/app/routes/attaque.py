...
async def attaque_sqli(SD_initial_id: int, type: TypeAttaqueResquest, db: Session = Depends(get_db)):
    print(f"[DEBUG] Received attack request for SD {SD_initial_id}")
    print(f"[DEBUG] Attack types: {type.attaque_type}")
    
    if run_attacks(SD_initial_id, type.attaque_type, db, True):
        print(f"[INFO] Attacks launched successfully")
