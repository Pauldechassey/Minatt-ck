...
def get_all_child_ids_recursively(
    initial_sous_domaine: SousDomaine, db: Session
) -> List[int]:
    logger.info(
        f"Démarrage de la récupération récursive des sous-domaines enfants pour l'ID: {initial_sous_domaine.id_SD}"
    )
    ...
    try:
        ...
        logger.info(
            f"Début du traitement avec l'ID initial: {initial_sous_domaine.id_SD} => {initial_sous_domaine.url_SD}"
        )
        ...
        while ids_to_process and safety_counter < max_iterations:
            ...
            children = (
                db.query(SousDomaine.id_SD)
                .filter(SousDomaine.id_SD_Sous_domaine == current_id)
                .all()
            )
        ...
        if safety_counter >= max_iterations:
            logger.warning(
                f"Atteinte du nombre maximum d'itérations ({max_iterations}). Possible boucle infinie évitée."
            )
