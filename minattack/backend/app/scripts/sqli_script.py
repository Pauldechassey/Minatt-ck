...
from minattack.backend.app.schemas.type_attaque_enum import TypeAttaque
...
class SQLIScanner:
        ...
        for field in form_details["inputs"]:
            if field["type"] not in ["submit", "button", "image"]:
                if field["name"] != input_field["name"]:
                    data[field["name"]] = self.get_garbage_value(
        ...
        if form_details["is_login_form"]:
            ...
            if username_field and password_field:
                if username_field != input_field["name"]:
                    data[username_field] = self.get_garbage_value(
                        "text", username_field
                    )
                if password_field != input_field["name"]:
                    data[password_field] = self.get_garbage_value(
        ...
        try:
            ...
            if not faille and form_details["is_login_form"]:
                ...
                # Vérification des nouveaux cookies (session potentielle créée)
                    ...
                    new_cookies = set(self.session.cookies.get_dict().keys()) - set(
                        initial_cookies.keys()
                    )
                    faille = True
            ...
            logger.warning(
                f"[ERREUR] Test échoué sur {url} avec {payload_attack}: {str(e)}"
            )
    ...
    def detecter_connexion_reussie(
        self, response, original_content, original_status_code, original_headers
    ):
