...
class CSRFScanner:
    def __init__(self):
        self.resultats = {"attaques": [], "failles": []}
    ...
    def test_csrf(self, url, timeout=10):
        ...
        self._analyze_forms(
            soup, url, has_csrf_token or has_csrf_header or has_csrf_cookie
        )
    ...
    def _check_page_for_csrf_token(self, soup):
        ...
        for script in scripts:
            if script.string and re.search(
                r"(csrf|xsrf|token|security|verify).*", script.string, re.I
            ):
                return True
        ...
        for hidden in hidden_inputs:
            if hidden.get("name") and any(
                keyword in hidden.get("name", "").lower()
                for keyword in self.csrf_keywords
            ):
                if (
                    hidden.get("value") and len(hidden.get("value", "")) > 8
                ):  # Token de longueur significative
                    return True
    ...
    def _analyze_forms(self, soup, url, has_global_csrf):
        ...
        for form in forms:
            ...
            if not has_form_csrf and not has_global_csrf:
                logger.warning(
                    f"[VULNÉRABLE] {url} - CSRF: POST method with sensitive fields without detectable CSRF protection"
                )
                attaque.resultat = 1
    ...
    def _check_form_for_csrf(self, form):
        ...
        for inp in inputs:
            ...
            if any(
                keyword in input_name or keyword in input_id
                for keyword in self.csrf_keywords
            ):
    ...
    def _is_search_form(self, form):
        ...
        if any(
            ind in form_action or ind in form_class or ind in form_id
            ...
            if (
                len(inputs) <= 2
            ):  # Un champ de recherche et possiblement un bouton submit
                return True
    ...
    def _has_sensitive_fields(self, form):
        ...
        for field in inputs:
            ...
            if any(
                ind in field_name or ind in field_id
                for ind in sensitive_field_indicators
            ):
                return True
    ...
    def _has_high_risk_fields(self, form):
        ...
        for field in inputs:
            ...
            if any(
                ind in field_name or ind in field_id for ind in high_risk_indicators
            ):
                return True
