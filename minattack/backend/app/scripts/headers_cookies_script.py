...
class HeadersCookiesScanner:
    ...
    def _analyze_headers(self, response, url):
        ...
        self._check_security_headers(
            headers, self.critical_headers, url, 6, is_critical=True
        )

        if is_https:
            self._check_security_headers(
                headers, self.recommended_headers, url, 4, is_critical=False
            )
            ...
            if header not in headers:
                ...
                if is_critical:
                    ...
                    self._log_vulnerability(
                        url, f"Header: {header}", description, id_prov, gravite
                    )
                    attaque.resultat = 1
                else:
                ...
                if not any(valid.lower() in header_value for valid in valid_values):
                    description = (
                        f"Misconfigured security header: {header}={headers[header]}"
                    )
                    self._log_vulnerability(
                        url, f"Header: {header}", description, id_prov, gravite
                    )
    ...
    def _analyze_cookies(self, response, url):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[ERREUR] Impossible de parser les cookies pour {url}: {e}"
                )
                return
        ...
        for cookie_name, cookie in cookies.items():
            ...
            is_sensitive = any(
                re.search(pattern, cookie_name, re.IGNORECASE)
            ...
            if is_sensitive:
                ...
                if samesite not in ["strict", "lax"]:
                    vulnerabilities.append(
                        "SameSite manquant ou faible (sensible à CSRF)"
                    )
            ...
            attaque = Attaque(
                payload=f"Test cookie {cookie_name}",
                date_attaque=datetime.now(),
                resultat=0,
                id_Type=4,
            )
            ...
            if vulnerabilities:
                ...
                self._log_vulnerability(
                    url, f"Cookie: {cookie_name}", description, id_prov, gravite
                )
                attaque.resultat = 1
