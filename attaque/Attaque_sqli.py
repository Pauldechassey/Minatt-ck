...
class Attaque_sqli:
    ...
    def test_field(self, url, form_details, input_field, payload):
        ...
        try:
            ...
            if not faille:
                # Vérification de la différence de longueur de la réponse
            ...
            if faille:
                print(f"[VULNÉRABLE] {url} - Champ {input_field['name']} vulnérable ({type_vuln}) avec {payload}")
                
                # Enregistrement des informations de réponse pour le rapport
                response_info = {
    ...
    def test_sqli(self, url, form_details):
        ...
        for input_field in form_details["inputs"]:
            ...
            for payload in self.payloads:
                self.test_field(url, form_details, input_field, payload)
                # Arrêt si une faille est détectée
                if any(result["champ"] == input_name for result in self.resultats):
                    break
